...
int main() {
    ...
    FILE* cenario = fopen("cenario.bin", "rb");
    ...
    fclose(cenario);
    FILE* caminho = fopen("caminho.bin", "rb");
    ...
    while(1) {
        ...
        geracoesRepetidas++;
        for(int i = 0; i < NUM_INDIVIDUOS; i++) {
            if((populacao[i]->distanciaDestino < melhorHistorico->distanciaDestino && populacao[i]->menorDistancia >= melhorHistorico->menorDistancia) || (populacao[i]->menorDistancia > melhorHistorico->menorDistancia && populacao[i]->distanciaDestino <= melhorHistorico->menorDistancia)) {
