...
#define GERACOES_REPETIDAS 20 //Quantas gerações com o melhor repetido para que a mutação varie 
...
void mutaIndividuo(Individuo* individuo, int mutacao) {
    for(int i = 0; i < mutacao; i++) {
    	int index = (rand()%NUM_JUNTAS);
    	float variacao = ((rand() % 101) / 100) * MAX_STEP;
        individuo->angulos[index] += rand() % 2 == 0 ? variacao : -variacao;
    }
}
